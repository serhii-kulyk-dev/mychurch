...
export default function SupportHero() {
  ...
  const h = c.hero;
  const mail = c.channels.items.find((i) => i.id === "mail");
  const telegram = c.channels.items.find((i) => i.id === "telegram");
    ...
    <section className="relative w-full overflow-hidden bg-surface flex flex-col items-center pt-14 md:pt-24 pb-14 md:pb-20">
      ...
      <div className="relative z-10 w-full max-w-[1120px] px-5 md:px-8 flex flex-col items-center gap-8 md:gap-10 text-center">
        <FadeIn className="flex flex-col items-center gap-4">
          ...
          <p className="text-[17px] md:text-[19px] text-ink-2 leading-[1.55] max-w-[660px]">{h.lead}</p>
        </FadeIn>
        ...
        <FadeIn delay={1} className="flex flex-col items-center gap-3">
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-center w-full sm:w-auto">
            <a
              href={telegram?.href ?? SITE_TELEGRAM}
              target="_blank"
            ...
            >
              <Send className="relative w-[17px] h-[17px] text-white" />
            ...
            >
              <Mail className="w-[16px] h-[16px] text-ink-2" />
              <span className="font-medium text-[16px] tracking-[-0.32px] leading-[1.4] whitespace-nowrap">
                {h.ctaSecondary}
              </span>
            </a>
          </div>
          <p className="text-[13.5px] text-ink-3 leading-[1.4]">{h.note}</p>
        </FadeIn>
          ...
          {h.facts.map((f, i) => (
            <Fragment key={f.label}>
              ...
              <div className="flex flex-col items-center gap-1">
                <span className="text-[22px] md:text-[26px] font-semibold text-ink leading-none tracking-[-0.7px]">
                  {f.value}
                </span>
