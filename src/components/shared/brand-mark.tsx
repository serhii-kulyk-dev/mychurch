/* Згенеровано `python3 brand/build.py` з `brand/gen.py` — руками не правимо.
   Знак бере колір з теми (`currentColor`), щоб на темному фоні світлішав
   разом із рештою бренду. Растр для OG і аватарок — у `public/`. */

export default function BrandMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 100 100" fill="none" className={className} aria-hidden>
      <path d="M41.31 30.50A11.06 11.06 0 1 0 36.85 19.75A2.76 2.76 0 0 1 32.51 21.55A11.06 11.06 0 0 0 17.08 36.98A2.76 2.76 0 0 1 15.28 41.32A11.06 11.06 0 0 0 15.28 63.15A2.76 2.76 0 0 1 17.08 67.49A11.06 11.06 0 0 0 32.51 82.92A2.76 2.76 0 0 1 36.85 84.72A11.06 11.06 0 0 0 58.68 84.72A2.76 2.76 0 0 1 63.02 82.92A11.06 11.06 0 0 0 78.45 67.49A2.76 2.76 0 0 1 80.25 63.15A11.06 11.06 0 1 0 69.50 58.69A2.76 2.76 0 0 1 67.70 63.04A11.06 11.06 0 0 0 58.57 72.17A2.76 2.76 0 0 1 54.23 73.97A11.06 11.06 0 0 0 41.31 73.97A2.76 2.76 0 0 1 36.96 72.17A11.06 11.06 0 0 0 27.83 63.04A2.76 2.76 0 0 1 26.03 58.69A11.06 11.06 0 0 0 26.03 45.77A2.76 2.76 0 0 1 27.83 41.43A11.06 11.06 0 0 0 36.96 32.30A2.76 2.76 0 0 1 41.31 30.50A11.06 11.06 0 0 0 41.31 30.50ZM71.89 17.06A11.06 11.06 0 1 0 94.00 17.06A11.06 11.06 0 1 0 71.89 17.06Z" fill="currentColor" />
    </svg>
  );
}
